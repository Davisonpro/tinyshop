function initRichEditor(editorEl, textareaEl, placeholder, opts) {
  ...
  return { setContent: function(html) {
...
(function($) {
  ...
  $("#fetchBtn").on("click", function() {
    ...
    fetchedData = null;
    _importVarGroups = [];
    TinyShop.api("POST", "/api/import/fetch", { url }).done(function(res) {
      ...
      resetBtn($btn, ".fetch-btn-label", ".fetch-btn-loading");
    });
  });
  $("#parseHtmlBtn").on("click", function() {
    ...
    fetchedData = null;
    _importVarGroups = [];
    TinyShop.api("POST", "/api/import/fetch", { html }).done(function(res) {
      fetchedData = res.product;
      renderPreview(res.product);
      $("#importPreview").show();
      $("#pasteHtml").val("");
      ...
      resetBtn($btn, ".parse-btn-label", ".parse-btn-loading");
    });
  });
  ...
  function readImportVarGroups() {
    ...
    $("#prevVariations .variation-group").each(function() {
      ...
      $(this).find(".variation-option-row").each(function() {
        ...
        if (val) {
          var o = { value: val };
          if (ps !== "") o.price = parseFloat(ps);
          opts.push(o);
        }
      });
      if (name && opts.length) _importVarGroups.push({ name, options: opts });
    });
  }
  ...
  $("#saveBtn").on("click", function() {
    ...
    TinyShop.api("POST", "/api/import/save", payload).done(function() {
  ...
  $("#quickFindBtn").on("click", function() {
    ...
    if (!text) {
      TinyShop.toast("Type some products first", "error");
      return;
    }
    var $btn = $(this);
    $btn.prop("disabled", true);
    ...
    _quickSavedTotal = 0;
    var cancelled = false;
    _quickAbort = function() {
      cancelled = true;
    };
    sipStep(1, "Reading your product list...");
    TinyShop.api("POST", "/api/import/smart-parse", { text }).done(function(res) {
      if (cancelled) {
        finish();
        return;
      }
      ...
      TinyShop.api("POST", "/api/import/smart-resolve", { items }).done(function(res2) {
        if (cancelled) {
          finish();
          return;
        }
  ...
  function renderQuickAccordion() {
    ...
    _quickResults.forEach(function(r, i) {
      var thumbHtml = r.images && r.images.length ? '<img src="' + escHtml(r.images[0]) + `" alt="" loading="lazy" onerror="this.parentNode.innerHTML='<div class=\\'qi-thumb-placeholder\\'><i class=\\'fa-solid fa-image\\'></i></div>'">` : '<div class="qi-thumb-placeholder"><i class="fa-solid fa-image"></i></div>';
      ...
      var itemId = "qi-" + i;
      var html = '<div class="qi-item" data-index="' + i + '" id="' + itemId + '"><div class="qi-header" role="button" tabindex="0" aria-expanded="false" aria-controls="' + itemId + '-body"><div class="qi-thumb">' + thumbHtml + '</div><div class="qi-header-info"><div class="qi-header-name">' + escHtml(r.name || "") + '</div><div class="qi-header-meta"><span class="qi-price-tag">' + CURRENCY + " " + priceDisplay + '</span><span class="smart-import-badge smart-import-badge-source">' + sourceLabel + "</span>" + (catLabel ? '<span class="smart-import-badge smart-import-badge-source">' + escHtml(catLabel) + "</span>" : "") + '</div></div><i class="fa-solid fa-chevron-down qi-chevron"></i></div><div class="qi-body" id="' + itemId + '-body" role="region" style="display:none"><div class="form-section"><div class="form-section-title">Product details</div><div class="form-group"><label>Title</label><input type="text" class="form-control qi-field" data-field="name" value="' + escHtml(r.name || "") + '"></div><div class="form-group"><label>Short description</label><div class="rich-editor qi-short-desc-editor"></div><textarea class="form-control qi-field" data-field="description" style="display:none">' + escHtml(r.description || "") + '</textarea></div><div class="form-group"><label>Product description</label><div class="rich-editor qi-full-desc-editor"></div><textarea class="form-control qi-field" data-field="full_description" style="display:none"></textarea></div></div><div class="form-section"><div class="form-section-title">Pricing</div><div class="form-group"><label>Selling price</label><div class="input-group"><span class="input-group-prefix">' + CURRENCY + '</span><input type="text" class="form-control price-input qi-field" data-field="price" value="' + (r.price || "") + '" inputmode="decimal"></div></div><div class="form-group"><label>Original price</label><div class="input-group"><span class="input-group-prefix">' + CURRENCY + '</span><input type="text" class="form-control price-input qi-field" data-field="compare_price" value="' + (r.compare_price && r.compare_price > (r.price || 0) ? r.compare_price : "") + '" placeholder="Leave empty if no discount" inputmode="decimal"></div><p class="form-hint">If set, customers see this crossed out next to the sale price</p></div></div><div class="form-section"><div class="form-section-title">Photos</div><p class="form-hint mb-md">First photo is the main one. Tap &times; to remove.</p><div class="image-gallery qi-images"></div></div><div class="form-section"><div class="form-section-title">Categories</div><div class="import-categories qi-categories"></div><div class="category-select-row" style="margin-top:12px"><div class="category-picker-btn qi-open-cat-picker"><span class="picker-placeholder">Select a category</span><i class="fa-solid fa-chevron-down" style="font-size:12px;color:#C7C7CC"></i></div><button type="button" class="btn-add-category qi-add-cat-btn" title="Add category"><i class="fa-solid fa-plus"></i></button></div></div><div class="form-section qi-var-section"' + (!(r.variations && r.variations.length) ? ' style="display:none"' : "") + `><div class="form-section-title">Options</div><p class="form-hint" style="margin-bottom:16px">Sizes, colors, or styles. You can edit, remove, or add new ones.</p><div class="qi-variations"></div><button type="button" class="variation-add-group qi-add-var-group"><i class="fa-solid fa-plus"></i> Add option like Size or Color</button></div><div class="form-section"><div class="form-section-title">Options</div><div class="form-toggle-row"><div><div class="form-toggle-label">Featured</div><p class="form-hint mt-xs">Show at the top of your store</p></div><label class="toggle-switch"><input type="checkbox" class="qi-field" data-field="is_featured"><span class="toggle-slider"></span></label></div><div class="form-toggle-row"><div><div class="form-toggle-label">Sold out</div><p class="form-hint mt-xs">Customers can't buy it</p></div><label class="toggle-switch"><input type="checkbox" class="qi-field" data-field="is_sold"><span class="toggle-slider"></span></label></div></div><div class="qi-actions"><button type="button" class="btn btn-primary qi-save-btn" data-index="` + i + '"><i class="fa-solid fa-check"></i> Save to Store</button><button type="button" class="btn btn-ghost qi-remove-btn" data-index="' + i + '" title="Remove"><i class="fa-solid fa-trash-can"></i></button></div></div></div>';
      $acc.append(html);
      ...
      $item.find(".price-input").each(function() {
  ...
  function renderQiImages($item, images) {
    ...
    images.forEach(function(src, i) {
      var $wrap = $('<div class="image-gallery-item" data-img-index="' + i + '"><img src="' + escHtml(src) + '" alt="" loading="lazy"><button type="button" class="image-gallery-remove qi-remove-img" data-img-index="' + i + '">&times;</button></div>');
      $wrap.find("img").on("error", function() {
        var imgs = $item.data("images") || [];
        imgs = imgs.filter(function(u) {
          return u !== $wrap.find("img").attr("src");
        });
  ...
  function readQiVariations($item) {
    ...
    $item.find(".qi-variations .variation-group").each(function() {
      ...
      if (name && opts.length) groups.push({ name, options: opts });
    });
    return groups;
  }
  ...
  $(document).on("click", ".qi-save-btn", function() {
    ...
    TinyShop.api("POST", "/api/import/smart-save", { products: [product] }).done(function(res) {
      if (res.saved_count > 0) {
        var msg = res.duplicates && res.duplicates.length ? product.name + " added (similar product already exists)" : product.name + " added!";
  ...
  $("#quickAddAllBtn").on("click", function() {
    ...
    if (!products.length) {
      TinyShop.toast("No valid products", "error");
      return;
    }
    ...
    TinyShop.api("POST", "/api/import/smart-save", { products }).done(function(res) {
      ...
      setTimeout(function() {
        $items.remove();
        showQuickSuccess(_quickSavedTotal);
      }, 300);
    }).fail(function() {
      TinyShop.toast("Failed to save products", "error");
    }).always(function() {
  ...
  function buildQiProduct($item) {
    ...
    if (!name) {
      TinyShop.toast("Product name is required", "error");
      return null;
    }
    if (price <= 0) {
      TinyShop.toast('Price must be greater than 0 for "' + name + '"', "error");
      return null;
    }
    ...
    $item.find(".qi-variations .variation-group").each(function() {
      ...
      var opts = [];
      $(this).find(".variation-option-value").each(function() {
        if ($(this).val().trim()) opts.push(1);
      });
      if (gname && !opts.length || !gname && opts.length) hasIncomplete = true;
    });
    if (hasIncomplete) {
      TinyShop.toast('Fix incomplete variations for "' + name + '"', "error");
      return null;
    }
    var cats = $item.data("categories") || [];
  ...
  function syncAllEditors($item) {
    $item.find(".rich-editor-content").each(function() {
      $(this).trigger("blur");
    });
  }
  ...
  function showQuickEmpty() {
    ...
    $acc.html(
      '<div class="qi-empty"><div class="qi-empty-icon"><i class="fa-solid fa-broom"></i></div><div class="qi-empty-text">All products removed</div><p class="qi-empty-hint">Edit your search below and try again with different products.</p><button type="button" id="quickRetryBtn" class="btn btn-primary qi-success-btn"><i class="fa-solid fa-arrow-rotate-left"></i> Try Again</button></div>'
    );
    $("#quickAddBulkBar").hide();
  }
  ...
  function renderCategoryChips($container, cats, removeClass) {
    ...
    cats.forEach(function(c, i) {
      $container.append(
        '<span class="import-cat-chip"><i class="fa-solid fa-tag"></i> ' + escHtml(c) + '<button type="button" class="import-cat-remove ' + removeClass + '" data-cat-index="' + i + '" data-index="' + i + '">&times;</button></span>'
      );
    });
  }
  function renderVariationGroups($container, groups) {
    ...
    (groups || []).forEach(function(group, gi) {
      var html = '<div class="variation-group" data-gidx="' + gi + '"><div class="variation-group-header"><input type="text" class="variation-group-name" placeholder="Option name (e.g. Size)" value="' + escHtml(group.name || "") + '" autocomplete="off"><button type="button" class="variation-group-remove" title="Remove">&times;</button></div><div class="variation-options">';
      (group.options || []).forEach(function(opt) {
        var pv = opt.price !== null && opt.price !== void 0 ? opt.price : "";
        html += '<div class="variation-option-row"><input type="text" class="variation-option-value" placeholder="Value" value="' + escHtml(opt.value || "") + '" autocomplete="off"><input type="text" class="variation-option-price price-input" placeholder="Price" inputmode="decimal" value="' + escHtml(String(pv)) + '" autocomplete="off"><button type="button" class="variation-option-remove" title="Remove">&times;</button></div>';
      });
      html += '</div><button type="button" class="variation-add-value">+ Add value</button></div>';
      $container.append(html);
    });
    $container.find(".price-input").each(function() {
      TinyShop.initPriceInput($(this));
    });
  }
  function appendVariationOption($optionsContainer) {
    $optionsContainer.append(
      '<div class="variation-option-row"><input type="text" class="variation-option-value" placeholder="Value" autocomplete="off"><input type="text" class="variation-option-price price-input" placeholder="Price" inputmode="decimal" autocomplete="off"><button type="button" class="variation-option-remove" title="Remove">&times;</button></div>'
    );
  ...
  function openCategoryPicker(existingCats, onSelect) {
    var html = '<div class="category-picker-search"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg><input type="text" id="catPickerSearch" placeholder="Search categories..." autocomplete="off"></div><div class="category-picker-list">';
    ...
    $("#catPickerSearch").on("input", function() {
      ...
      _t = setTimeout(function() {
        ...
        if (!q) {
          $list.find(".category-picker-group, .category-picker-item").show();
          return;
        }
  ...
  function buildPickerItem(name, existing, isParent) {
    ...
    return '<div class="category-picker-item ' + cls + (sel ? " selected" : "") + '" data-name="' + escHtml(name) + '" data-search-name="' + escHtml(name.toLowerCase()) + '"><span>' + escHtml(name) + '</span><span class="category-picker-check"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"><polyline points="20 6 9 17 4 12"/></svg></span></div>';
  }
  function openNewCategoryModal(onCreated) {
    var html = '<form id="newCatForm" autocomplete="off"><div class="form-group"><label for="newCatName">Category Name</label><input type="text" class="form-control" id="newCatName" placeholder="e.g. Accessories" required autofocus autocomplete="off"></div><button type="submit" class="btn btn-primary" id="saveCatBtn" style="width:100%;min-height:52px;font-size:1rem;font-weight:600;border-radius:14px">Add Category</button></form>';
    ...
    $("#newCatForm").on("submit", function(e) {
      ...
      TinyShop.api("POST", "/api/import/save-category", { name }).done(function(res) {
