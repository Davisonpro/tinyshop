...
<div class="product-page">
        ...
        <div class="product-gallery" id="productGallery">
            ...
            {else}
                <div class="product-gallery-empty">
                    <i class="fa-solid fa-image" style="font-size:48px;color:#555"></i>
                </div>
            {/if}
    ...
    <div class="container">
        <div class="more-products">
            <h2 class="more-products-title">More</h2>
...
(function() {ldelim}
    ...
    if (desc && toggle) {ldelim}
        if (desc.scrollHeight > desc.offsetHeight + 4) {ldelim}
            desc.classList.add('has-overflow');
            toggle.style.display = '';
            toggle.addEventListener('click', function() {ldelim}
    ...
    function updateIndicators(idx) {ldelim}
        allDots.forEach(function(d, i) {ldelim}
            d.classList.toggle('active', i === idx);
        {rdelim});
        allThumbs.forEach(function(t, i) {ldelim}
            t.classList.toggle('active', i === idx);
        {rdelim});
    {rdelim}

    var scrollTimer;
    track.addEventListener('scroll', function() {ldelim}
        clearTimeout(scrollTimer);
        scrollTimer = setTimeout(function() {ldelim}
    ...
    if (dots) {ldelim}
        dots.addEventListener('click', function(e) {ldelim}
            var dot = e.target.closest('.gallery-dot');
            if (!dot) return;
            var idx = parseInt(dot.dataset.index, 10);
            track.scrollTo({ldelim} left: idx * track.offsetWidth, behavior: 'smooth' {rdelim});
        {rdelim});
    {rdelim}

    if (thumbs) {ldelim}
        thumbs.addEventListener('click', function(e) {ldelim}
