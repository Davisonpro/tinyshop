...
<div class="shop-page" data-subdomain="{$shop.subdomain|escape}" data-total="{$total_products}" data-limit="{$products_limit}" data-currency="{$currency_symbol|escape}">
    <div class="container">
        ...
        <div class="shop-search" id="shopSearch">
            ...
            <input type="text" class="shop-search-input" id="searchInput" placeholder="Search our collection..." autocomplete="off">
        ...
        <div class="search-empty" id="searchEmpty" style="display:none">
            ...
            <p class="search-empty-hint">Try searching for something else</p>
        </div>

        <section class="product-grid ember-grid-staggered" id="catalogue">
            ...
            {foreachelse}
                <div class="empty-state">
                    ...
                    <p class="empty-state-title"><strong>We're getting things ready</strong></p>
                    <p class="empty-state-text">Beautiful things are on their way — check back soon!</p>
                </div>
            {/foreach}
        </section>
...
{* Ember theme renderer — info + arrow layout in card body *}
<script>
...
window.TinyShopTheme.renderProductCard = function(p, currencySymbol) {ldelim}
    ...
    return '<a href="/' + slug + '" class="product-card' + soldClass + '" data-category="' + (p.category_id || '') + '">'
        ...
        + '</div>'
        + '<div class="product-card-arrow"><i class="fa-solid fa-arrow-right" style="font-size:18px"></i></div>'
        + '</div></a>';
{rdelim};
</script>
