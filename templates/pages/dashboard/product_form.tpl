...
<form id="productForm" class="dash-form" autocomplete="off">
    ...
    <div class="form-section">
        ...
        <div class="form-group">
            ...
            <div class="rich-editor" id="shortDescEditor"></div>
            <textarea class="form-control" id="productShortDesc" name="description" style="display:none">{if $is_edit}{$product.description|escape}{/if}</textarea>
        </div>
        <div class="form-group">
            <button type="button" class="desc-toggle{if $is_edit && $product.full_description} open{/if}" id="longDescToggle">
                <i class="fa-solid fa-plus toggle-icon-add"></i>
                <i class="fa-solid fa-minus toggle-icon-remove"></i>
                <span>Product description</span>
            </button>
            <p class="form-hint" id="longDescHint"{if $is_edit && $product.full_description} style="display:none"{/if}>Optional — add a detailed description for your product page</p>
            <div class="desc-toggle-body{if !$is_edit || !$product.full_description} hidden{/if}" id="longDescBody">
                <div class="rich-editor" id="richEditor"></div>
                <textarea class="form-control" id="productDesc" name="full_description" style="display:none">{if $is_edit}{$product.full_description|escape}{/if}</textarea>
            </div>
        </div>
        <div class="form-group">
...
<script>
{literal}
// Reusable rich text editor factory
function initRichEditor(editorId, textareaId, placeholder) {
    ...
    actions.forEach(function(a, i) {
        if (i === 3) { var sep = document.createElement('div'); sep.className = 'rich-editor-sep'; toolbar.appendChild(sep); }
    ...
    function updateActiveStates() {
        buttons.forEach(function(b) {
            ...
            else if (b.action.cmd === 'bold' || b.action.cmd === 'italic') { active = document.queryCommandState(b.action.cmd); }
            else if (b.action.cmd === 'insertUnorderedList' || b.action.cmd === 'insertOrderedList') { active = document.queryCommandState(b.action.cmd); }
    ...
    content.addEventListener('paste', function(e) {
        ...
        if (html) {
            ...
            tmp.innerHTML = html;
            tmp.querySelectorAll('*').forEach(function(el) { el.removeAttribute('style'); el.removeAttribute('class'); el.removeAttribute('id'); });
            var allowed = ['P','BR','B','STRONG','I','EM','UL','OL','LI','H2','H3','A'];
            tmp.querySelectorAll('*').forEach(function(el) { if (allowed.indexOf(el.tagName) === -1) el.replaceWith(document.createTextNode(el.textContent)); });
    ...
    return { setContent: function(html) { content.innerHTML = html || ''; sync(); }, contentEl: content };
}

// Init short description editor
var shortDescRich = initRichEditor('shortDescEditor', 'productShortDesc', 'Brief overview for search results and social sharing...');

// Init product description editor
var descRich = initRichEditor('richEditor', 'productDesc', 'Tell your customers about this product...');

// Expose for draft restore
if (shortDescRich) window._setShortDescContent = shortDescRich.setContent;
if (descRich) window._setEditorContent = descRich.setContent;

// Product description toggle
(function() {
    var toggle = document.getElementById('longDescToggle');
    var body = document.getElementById('longDescBody');
    var hint = document.getElementById('longDescHint');
    if (!toggle || !body) return;
    toggle.addEventListener('click', function() {
        var opening = !toggle.classList.contains('open');
