...
$(function() {ldelim}
    ...
    function startPayment(planId, planName, cycle, gateway, $btn) {ldelim}
        var payload = {ldelim}
            plan_id: planId,
            cycle: cycle,
            gateway: gateway
        ...
        if (gateway === 'mpesa') {ldelim}
            ...
            if (!payload.mpesa_phone) {ldelim}
                ...
                if ($btn) $btn.prop('disabled', false).text($btn.data('original-text'));
                return;
            {rdelim}
        {rdelim}
        ...
        TinyShop.api('POST', '/api/billing/subscribe', payload).done(function(res) {ldelim}
            if (res.gateway === 'mpesa' && res.poll_url) {ldelim}
                var waitHtml = '<div class="mpesa-wait">' +
                    '<div class="mpesa-wait-spinner btn-spinner"></div>' +
                    '<div class="mpesa-wait-title">Check your phone</div>' +
                    '<div class="mpesa-wait-desc">Enter your M-Pesa PIN when prompted.<br>This page will update automatically.</div>' +
                    '</div>';
    ...
    $('.plan-choose-btn').on('click', function() {ldelim}
        ...
        if (gateways.length === 1) {ldelim}
            if (gateways[0] === 'mpesa') {ldelim}
                ...
                var phoneHtml = '<div class="gateway-summary">Subscribe to <strong>' + planName + '</strong> (' + cycle + ')</div>'
                    + '<div class="gateway-phone-field">'
                    + '<label class="gateway-phone-label">M-Pesa Phone Number</label>'
                    + '<input type="tel" id="billingMpesaPhoneInput" class="form-control" placeholder="e.g. 0712 345 678" inputmode="numeric">'
                    + '</div>'
                    + '<button type="button" class="btn-block btn-accent gateway-confirm-btn" id="confirmMpesaPhoneBtn">Pay via M-Pesa</button>';
                TinyShop.openModal('Payment Method', phoneHtml);
                $('#confirmMpesaPhoneBtn').on('click', function() {ldelim}
                    var $b = $(this);
                    $b.data('original-text', $b.text());
                    $b.prop('disabled', true).html('<span class="btn-spinner"></span> Processing...');
                    startPayment(planId, planName, cycle, 'mpesa', $b);
                {rdelim});
                return;
            {rdelim}
            var $b = $(this);
            $b.data('original-text', $b.text());
            $b.prop('disabled', true).html('<span class="btn-spinner"></span> Processing...');
            startPayment(planId, planName, cycle, gateways[0], $b);
            return;
        {rdelim}
        ...
        for (var i = 0; i < gateways.length; i++) {ldelim}
            ...
            {rdelim} else if (gw === 'pesapal') {ldelim}
                iconHtml = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="12" fill="#0A7BC4"/><text x="12" y="16" text-anchor="middle" font-size="9" font-weight="bold" fill="white">PP</text></svg>';
                label = 'Pesapal'; desc = 'Pay with Pesapal';
            {rdelim} else {ldelim}
        ...
        html += '</div>'
            + '<div class="gateway-phone-field" id="billingMpesaPhone" style="display:none">'
            + '<label class="gateway-phone-label">M-Pesa Phone Number</label>'
            + '<input type="tel" id="billingMpesaPhoneInput" class="form-control" placeholder="e.g. 0712 345 678" inputmode="numeric">'
            + '</div>'
            + '<button type="button" class="btn-block btn-accent gateway-confirm-btn" id="confirmGatewayBtn">Continue</button>';

        TinyShop.openModal('Payment Method', html);

        // Show phone input if M-Pesa is pre-selected
        if (gateways[0] === 'mpesa') {ldelim}
            $('#billingMpesaPhone').show();
        {rdelim}
        ...
        $('#confirmGatewayBtn').on('click', function() {ldelim}
            ...
            $b.data('original-text', $b.text());
            $b.prop('disabled', true).html('<span class="btn-spinner"></span> Processing...');
            startPayment(planId, planName, cycle, selectedGw, $b);
        {rdelim});
    {rdelim});
