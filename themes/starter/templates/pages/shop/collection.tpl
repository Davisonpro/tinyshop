...
<main class="shop-content" data-subdomain="{$shop.subdomain|escape}" data-total="{$total_products}" data-limit="{$products_limit}" data-currency="{$currency_symbol|escape}" data-active-category="{$category.id}">
    ...
    <div class="collection-hero">
        <nav class="collections-breadcrumb">
            ...
            <span class="breadcrumb-sep">/</span>
            <span class="breadcrumb-current">{$category.name|escape}</span>
        </nav>
        ...
        <div class="collection-hero-banner">
            ...
            <div class="collection-hero-overlay">
                ...
                <p class="collection-hero-count">{$total_products} {if $total_products == 1}item{else}items{/if}</p>
            </div>
        </div>
        {else}
    ...
    <div class="load-more-wrap">
        <button class="load-more-btn" id="loadMoreBtn">
            Show more <span class="load-more-count">({$total_products - $products_limit} more)</span>
        </button>
    </div>
    {/if}
    ...
    function fetchProducts(append) {ldelim}
        ...
        fetch(url)
            ...
            .then(function(data) {ldelim}
                ...
                if (countEl) {ldelim}
                    countEl.textContent = state.total + (state.total === 1 ? ' item' : ' items');
                {rdelim}
                ...
                if (remaining > 0) {ldelim}
                    if (!loadMoreWrap) {ldelim}
                        ...
                        wrap.innerHTML = '<button class="load-more-btn" id="loadMoreBtn">Show more <span class="load-more-count">(' + remaining + ' more)</span></button>';
